...
static int power_gpio_requested;

static int mmc1_set_power(struct device *dev, int slot, int power_on, int vdd)
{
	int err;

	if (!power_gpio_requested) {
		err = gpio_request(64, "MMC PWR EN");
		if (err)
			pr_err("CM-T3517: failed req mmcpwren gpio: %d\n", err);

		power_gpio_requested = 1;
	}
	gpio_direction_output(64, power_on ? 0 : 1);
...
void __init am3517_mmc_init(struct am3517_hsmmc_info *controllers)
{
	...
	for (c = controllers; c->mmc; c++) {
		...
		if (c->mmc == 1) {
			...
			mmc->slots[0].ocr_mask = MMC_VDD_32_33 | MMC_VDD_33_34;
