...
static struct gpio firecracker_gpios[] __initdata = {
	...
	{ FIRECRACKER__O_STATUS_GREEN_2, GPIOF_OUT_INIT_LOW, "FIRECRACKER__O_STATUS_GREEN_2" },
	{ FIRECRACKER__O_STATUS_GREEN_3, GPIOF_OUT_INIT_LOW, "FIRECRACKER__O_STATUS_GREEN_3" },
...
static void __init firecracker_gpios_init(void)
{
	if (gpio_request_array(firecracker_gpios, ARRAY_SIZE(firecracker_gpios))) {
		printk(KERN_ERR "FAILED to obtain Firecracker GPIOs!\n");
		return;
	}
	...
	// uart 0 (sat cmd) control lines
	if ( gpio_export(FIRECRACKER__I_SAT_CTL_DSR, 0) < 0 )
	{
		printk(KERN_ERR "gpio failed to export 'FIRECRACKER__I_SAT_CTL_DSR'\n");
		return;
	}
	if ( gpio_export(FIRECRACKER__O_SAT_CTL_DTR, 0) < 0 )
	{
		printk(KERN_ERR "gpio failed to export 'FIRECRACKER__O_SAT_CTL_DTR'\n");
		return;
	}
